...
class IntegratedODEInterface:
    ...
    def run(self):
        ...
        # Add a new "Dataset Management" entry so users can manage their current
        # session dataset.  This page allows users to view, export, import and
        # upload datasets to the backend API.
    ...
    def dataset_management_page(self):
        """View and manage the current session dataset"""
        st.title("📦 Dataset Management")

        dataset = st.session_state.current_dataset
        # Show notice if there is nothing to manage
        if not dataset:
            st.info("Current session dataset is empty. Generate or load ODEs first to manage them.")
        else:
            ...
            # Summary metrics for quick insight
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Total ODEs", len(df))
            with col2:
                unique_gens = df['generator'].nunique() if 'generator' in df.columns else 'N/A'
                st.metric("Unique Generators", unique_gens)
            with col3:
                if 'verified' in df.columns:
                    verified_rate = (df['verified'].sum() / len(df) * 100)
                    st.metric("Verified", f"{verified_rate:.1f}%")
            ...
            # Actions for dataset management
            colA, colB, colC, colD = st.columns(4)
            with colA:
                if st.button("🗑️ Clear Dataset"):
                    st.session_state.current_dataset = []
                    st.success("Cleared current session dataset.")
            ...
            with colD:
                ...
                if st.button("📤 Upload to API"):
                    # Upload dataset to backend via API
                    with st.spinner("Uploading dataset to API..."):
                        resp = self._call_api_create_dataset(dataset, dataset_name)
                        if resp and resp.get('status') == 'success':
                            st.success(f"Dataset '{dataset_name}' created on server.")
                        else:
                            st.error(f"Failed to create dataset: {resp.get('error') if resp else 'Unknown error'}")

        st.markdown("---")
        st.subheader("Load Dataset From File")
        uploaded_file = st.file_uploader("Upload JSON or CSV file", type=['json', 'csv'])
        if uploaded_file is not None:
            try:
                import io
                if uploaded_file.name.lower().endswith('.json'):
                    # Expect a list of ODE dictionaries in JSON
                    content = uploaded_file.read().decode('utf-8')
                    loaded_data = json.loads(content)
                    if isinstance(loaded_data, list):
                        st.session_state.current_dataset = loaded_data
                        st.success(f"Loaded {len(loaded_data)} ODEs from JSON file.")
                        st.experimental_rerun()
                    else:
                        st.error("JSON file must contain a list of ODE objects.")
                else:
                    # CSV parsing
                    df_loaded = pd.read_csv(uploaded_file)
                    st.session_state.current_dataset = df_loaded.to_dict(orient='records')
                    st.success(f"Loaded {len(df_loaded)} ODEs from CSV file.")
                    st.experimental_rerun()
            except Exception as e:
                st.error(f"Failed to load dataset: {e}")

    def _call_api_create_dataset(self, odes, dataset_name: str):
        """Create a dataset on the API backend

        Parameters
        ----------
        odes: list
            A list of ODE dictionaries to upload.
        dataset_name: str
            The name to assign to the dataset on the server.

        Returns
        -------
        dict
            A dictionary with 'status' and optionally 'data' or 'error'.
        """
        payload = {
            "odes": odes,
        ...
        try:
            response = requests.post(
                f"{API_BASE_URL}/datasets/create",
                headers=self.api_headers,
                json=payload,
                timeout=30,
            )
