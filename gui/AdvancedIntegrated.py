...
def render_analysis():
   ...
   if 'verified' in df and 'generator' in df:
       ...
       verification_by_gen = df.groupby('generator')['verified'].agg(['mean', 'count'])
       verification_by_gen['percentage'] = verification_by_gen['mean'] * 100
       
       fig = px.bar(
           verification_by_gen.reset_index(),
           x='generator',
           y='percentage',
           title="Verification Rate by Generator (%)",
           labels={'percentage': 'Verification Rate (%)'},
           text='percentage'
       )
   ...
   if any('ml_generated' in str(ode) for ode in all_odes):
       st.subheader("ML vs Traditional Generation")
       
       ml_odes = [ode for ode in all_odes if ode.get('model_type') or ode.get('ml_generated')]
       traditional_odes = [ode for ode in all_odes if not (ode.get('model_type') or ode.get('ml_generated'))]
       
       comparison_data = {
           'Type': ['Traditional', 'ML-Generated'],
           'Count': [len(traditional_odes), len(ml_odes)],
           'Verified': [
               sum(1 for ode in traditional_odes if ode.get('verified', False)),
               sum(1 for ode in ml_odes if ode.get('verified', False))
           ]
       }
       
       comparison_df = pd.DataFrame(comparison_data)
       comparison_df['Verification Rate'] = comparison_df['Verified'] / comparison_df['Count'] * 100
       
       fig = go.Figure()
       fig.add_trace(go.Bar(name='Total', x=comparison_df['Type'], y=comparison_df['Count']))
       fig.add_trace(go.Bar(name='Verified', x=comparison_df['Type'], y=comparison_df['Verified']))
       fig.update_layout(
           title="Traditional vs ML-Generated ODEs",
           barmode='group',
           yaxis_title="Count"
       )
       st.plotly_chart(fig, use_container_width=True)
   ...
   with col1:
       if st.button("Export as JSON"):
           json_str = json.dumps(all_odes, indent=2, default=str)
           b64 = base64.b64encode(json_str.encode()).decode()
           href = f'<a href="data:file/json;base64,{b64}" download="ode_analysis.json">Download JSON</a>'
   ...
   with col2:
       if st.button("Export as CSV"):
           ...
           href = f'<a href="data:file/csv;base64,{b64}" download="ode_analysis.csv">Download CSV</a>'
           st.markdown(href, unsafe_allow_html=True)
   
   with col3:
       if st.button("Generate Report"):
           report = generate_analysis_report(df, all_odes)
           b64 = base64.b64encode(report.encode()).decode()
           href = f'<a href="data:text/html;base64,{b64}" download="ode_report.html">Download Report</a>'
           st.markdown(href, unsafe_allow_html=True)
...
def render_documentation():
   ...
   with doc_tabs[0]:
       ...
       The ODE Master Generator System follows a complete workflow from generation to ML:
       
       1. **Generate Batch Dataset** 📦
          - Select multiple generators and functions
          - Configure parameter ranges
          - Generate diverse ODE dataset
       
       2. **Create Training Dataset** 💾
          - Save generated ODEs as named dataset
          - Prepare for ML training
       
       3. **Train ML Model** 🤖
          - Choose model architecture (PatternNet, Transformer, VAE)
          - Configure training parameters
          - Train on your dataset
       
       4. **Generate Novel ODEs** 🧪
          - Use trained model to generate new equations
          - Control creativity with temperature
          - Target specific styles
       
       ### Quick Start
       
       1. Go to **Complete Workflow** page
       ...
       - Start with a diverse dataset (mix of linear and nonlinear)
       - Use at least 100 ODEs for training
       - Higher epochs = better learning (but slower)
       - Temperature controls creativity (0.8 is balanced)
       """)
   ...
   with doc_tabs[1]:
       ...
       **L1: Second-Order Linear**
       - Form: `y''(x) + y(x) = RHS`
       - Applications: Harmonic oscillators, wave equations
       - Best for: Simple periodic solutions
       
       **L2: Damped Linear**
       - Form: `y''(x) + y'(x) = RHS`
       - Applications: Damped oscillations
       - Best for: Systems with friction
       
       **L3: First-Order Coupled**
       - Form: `y(x) + y'(x) = RHS`
       - Applications: RC circuits, growth models
       - Best for: Exponential behaviors
       
       **L4: Pantograph Equation**
       - Form: `y''(x) + y(x/a) - y(x) = RHS`
       - Applications: Delay differential equations
       - Best for: Systems with memory
       ...
       **N7: Composite Nonlinearity**
       ...
       - Applications: Customizable systems
       - Flexible nonlinear forms
       """)
   ...
   with doc_tabs[3]:
       ...
       **Parameters**:
       ...
       - Dropout rate: 0.2
       - Embeddings for generators and functions
       
       ### Transformer
       
       **Architecture**: Attention-based sequence model
       
       **Best for**:
       - Understanding ODE syntax
       - Generating well-formed equations
   ...
   with doc_tabs[4]:
       ...
       All endpoints return JSON with status and results.
       
       ### Rate Limits
       - 100 requests per minute
       - Large batch operations count as multiple requests
       """)
   ...
   with doc_tabs[5]:
       ...
       - Verify API is online at https://ode-api-production.up.railway.app/health
       - Check API key is correct
       
       #### Generation Fails
       - Some generator-function combinations may not work
       ...
       - Use PatternNet for faster training
       
       #### Low Verification Rate
       - Normal for complex nonlinear ODEs
       - Try simpler generators (L1, L2)
       - Adjust parameters closer to 0
       
       #### ML Generation Produces Invalid ODEs
...
def generate_analysis_report(df: pd.DataFrame, all_odes: List[Dict]) -> str:
   """Generate comprehensive HTML analysis report"""
   
   # Calculate statistics
   total_odes = len(df)
   verified_count = df['verified'].sum() if 'verified' in df else 0
   verification_rate = verified_count / total_odes if total_odes > 0 else 0
   
   # Generator statistics
   generator_stats = ""
   if 'generator' in df:
       for gen, count in df['generator'].value_counts().items():
           gen_verified = df[df['generator'] == gen]['verified'].sum() if 'verified' in df else 0
           gen_rate = gen_verified / count if count > 0 else 0
           generator_stats += f"""
           <tr>
               <td>{gen}</td>
               <td>{count}</td>
   ...
   function_stats = ""
   if 'function' in df:
       for func, count in df['function'].value_counts().head(10).items():
           func_verified = df[df['function'] == func]['verified'].sum() if 'verified' in df else 0
           func_rate = func_verified / count if count > 0 else 0
           function_stats += f"""
           <tr>
               <td>{func}</td>
               <td>{count}</td>
   ...
   <html>
   <head>
       <title>ODE Analysis Report</title>
       <style>
           ...
           .metric {{ display: inline-block; margin: 10px; padding: 15px; background-color: #ecf0f1; border-radius: 5px; text-align: center; min-width: 150px; }}
           ...
           .timestamp {{ text-align: center; color: #95a5a6; margin-top: 30px; }}
       </style>
   </head>
   <body>
       <div class="container">
           ...
           <div class="section">
               ...
               <div class="metrics">
                   <div class="metric">
                       <div class="metric-value">{total_odes}</div>
                       <div class="metric-label">Total ODEs</div>
                   </div>
                   <div class="metric">
                   ...
                   <div class="metric">
                       <div class="metric-value">{verification_rate:.1%}</div>
